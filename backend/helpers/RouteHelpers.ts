...
// generate route data with start, end, and stops formatted for response
export async function generateRouteData(
  ...
  if (originData.length === 0) {
    throw new Error("Could not locate start city");
  }
  ...
  if (destinationData.length === 0) {
    throw new Error("Could not locate end city");
  }

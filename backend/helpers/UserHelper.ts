import { RouteDBEntryWithID } from "../interfaces/RouteInterfaces";
...
export async function getUserRoutesFromDb(userID: string): Promise<object[]> {
  ...
  const collection = db.collection<RouteDBEntryWithID>(ROUTES_COLLECTION_NAME);

  const routes: RouteDBEntryWithID[] = await collection
    .find({ userID })
    .toArray();
